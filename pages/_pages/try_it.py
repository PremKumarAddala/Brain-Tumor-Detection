...
@st.cache_resource  # Updated cache decorator for models
def load_model():
...
def main():
    ...
    st.write(
        ...
        The program will automatically crop the image to the brain area and then analyze 
        it. The results will be displayed in the browser."""
    )
    ...
    if image_bytes:
        ...
        if image is None:
            st.error("Error: Could not load the image. Please upload a valid MRI scan.")
            return  # Stop execution if the image is invalid

        image = cv2.resize(image, (128, 128))

        st.write("#### Brain MRI scan image")
        st.image(image)
        ...
        with st.spinner(text="Analyzing..."):
            ...
            try:
                ...
                if img is None or img.size == 0:
                    st.error("Error: Image processing failed. Please try another image.")
                    return

                # Save the processed image safely
                if not cv2.imwrite("temp.png", img):
                    st.error("Error: Failed to save the processed image.")
                    return
                ...
                img_resized = cv2.resize(img, (50, 50))  # Ensure size matches training input
                ...
                if confidence >= 0.5:
                    st.success(f"The sample has a tumor with confidence: {confidence * 100:.2f}%")
                else:
                    st.success(f"The sample has no tumor with confidence: {(1 - confidence) * 100:.2f}%")
